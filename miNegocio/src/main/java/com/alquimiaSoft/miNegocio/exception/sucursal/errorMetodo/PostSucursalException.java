package com.alquimiaSoft.miNegocio.exception.sucursal.errorMetodo;

import com.alquimiaSoft.miNegocio.exception.CustomException;
import com.alquimiaSoft.miNegocio.exception.constants.ExceptionCode;
import org.springframework.http.HttpStatus;

public class PostSucursalException extends CustomException {

    public PostSucursalException(Throwable throwable){
        super(throwable);
    }

    @Override
    public String getCode(){
        return ExceptionCode.CODE_10;
    }

    @Override
    public String getDescription(){
        return ExceptionCode.MESSAGE_10;
    }

    @Override
    public HttpStatus getHttpStatus(){
        return HttpStatus.INTERNAL_SERVER_ERROR;
    }
}
